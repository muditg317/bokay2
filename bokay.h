...
bool bokay_interpret(BokayEngine *b) {
  ...
  Expr e;
  Value v;
  while (parser_get_expression(&b->p, &e)) {
    // parser_diag_expr(INFO, p, e);
    if (!bokay__eval_expr(e, &v)) {
      log(ERROR, LEX_LOC_Fmt ": Failed to evaluate expression %s", LEX_LOC_Arg(e.loc), expr_kind_to_str(e.kind));
      return_defer(false);
    }
  }
